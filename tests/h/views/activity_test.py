...
from h.models.group import ReadableBy, JoinableBy
...
class TestGroupSearchController(object):
    ...
    @pytest.mark.parametrize('test_group,test_user',
                             [('group', 'member'),
                              ('restricted_group', None), ('restricted_group', 'user'),
                              ('open_group', None), ('open_group', 'user')],
                             indirect=['test_group', 'test_user'])
    def test_returns_search_results_when_user_has_read_permission(
            self,
            controller,
            factories,
            test_group,
        ...
        # todo: put some has attrs in here
    ...
    def test_search_does_not_show_the_edit_link_to_non_admin_users(self,
                                                                   ...
                                                                   test_user,
                                                                   pyramid_request):
        # todo: update fake permission for admins
        result = controller.search()
    ...
    @pytest.mark.parametrize('test_group,test_user',
                             [('group', 'user')],
                              indirect=['test_group', 'test_user'])
    def test_join_adds_group_member(self, controller, test_group, test_user,
                                    pyramid_request, pyramid_config, group_service):
        pyramid_config.testing_securitypolicy(test_user)
        controller.join()

        group_service.member_join.assert_called_once_with(test_group, test_user)

    @pytest.mark.parametrize('test_group,test_user',
                             [('group', 'user')],
                              indirect=['test_group', 'test_user'])
    def test_join_redirects_to_search_page(self, controller, test_group, test_user,
                                           pyramid_request):
    ...
    @pytest.mark.parametrize('test_group, test_user, test_heading, test_subtitle, test_share_msg',
                             [('group', 'member', 'Members',
                               'Invite new members', 'Sharing the link lets people join this group:'),
                              ('open_group', None, 'Members',
                               'Share group', 'Sharing the link lets people view this group:'),
                              ('restricted_group', None, 'Members',
                                'Share group', 'Sharing the link lets people view this group:')],
                             indirect=['test_group', 'test_user'])
    ...
    def test_toggle_user_facet_returns_a_redirect(self, controller, test_group, test_user,
                                                  pyramid_request):
    ...
    def test_toggle_user_facet_preserves_query_when_removing_one_of_multiple_username_facets(
            self, controller, test_group, test_user, pyramid_request, toggle_user_facet_request):
    ...
    @pytest.mark.parametrize('q,test_group,test_user',
                             [('  user:fred   ', 'group', 'member')],
                             indirect=['test_group'])
    ...
    def users(self, request, user, factories):
        ...
        return {None: None,
                ...
                "user": factories.User(),
                "alt_authority_user": factories.User(authority='altauth.com')}
    ...
    def pyramid_request(self, request, group, pyramid_request):
        ...
        def fake_has_permission(permission, context=None):
            if permission == 'read':
                if (test_group.readable_by == ReadableBy.members and
                        pyramid_request.user not in test_group.members):
                    return False
                return True
            elif permission == 'join':
                if (test_group.joinable_by == JoinableBy.authority and
                        pyramid_request.user and
                        pyramid_request.user.authority == test_group.authority):
                    return True
                return False
            elif permission == 'admin':
                if test_group.creator == pyramid_request.user:
                    return True
                return False
