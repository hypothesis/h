...
class TestGroupCreateController(object):
    ...
    def test_post_redirects_to_list_view_on_success(
        self, pyramid_request, matchers, routes, handle_form_submission, default_org
    ):
    ...
    def test_post_creates_group_on_success(
        ...
        expected_userid = User(
            username=creator, authority=pyramid_request.authority
        ).userid
        ...
        group_svc.add_members.assert_called_once_with(
...
class TestGroupEditController(object):
    ...
    def test_update_updates_group_members_on_success(
        self, factories, pyramid_request, group_svc, user_svc, handle_form_submission
    ):
        group = factories.RestrictedGroup(pubid="testgroup")
        ...
        group_svc.update_members.assert_any_call(
...
def CreateAdminGroupSchema(patch):  # noqa: N802
    ...
    CreateAdminGroupSchema = patch(
        "h.views.admin_groups.CreateAdminGroupSchema"
    )  # noqa: N806
