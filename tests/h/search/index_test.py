...
class TestIndex(object):
    def test_annotation_ids_are_used_as_elasticsearch_ids(
        ...
        result = es_client.conn.get(
            index=es_client.index, doc_type="annotation", id=annotation.id
        )
        assert result["_id"] == annotation.id

    def test_it_can_index_an_annotation_with_no_document(self, factories, index, get):
    ...
    def test_it_indexes_the_annotations_document_web_uri(self, factories, index, get):
        annotation = factories.Annotation.build(
        ...
        assert (
            get(annotation.id)["document"]["web_uri"]
    ...
    def test_it_indexes_the_annotations_document_title(self, factories, index, get):
        annotation = factories.Annotation.build(
    ...
    def test_it_notifies(
        ...
        AnnotationTransformEvent.assert_called_with(
    ...
    def test_you_can_make_aggregations_on_tags_raw(
        self, es_client, factories, index, search
    ):
    ...
    def get(self, es_client):
        def _get(annotation_id):
            ...
            return es_client.conn.get(
                index=es_client.index, doc_type="annotation", id=annotation_id
            )["_source"]
...
def search(es_client):
    return elasticsearch1_dsl.Search(
        using=es_client.conn, index=es_client.index
    ).fields([])
