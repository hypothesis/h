...
class TestIndexAnnotation:
    ...
    def test_it_creates_an_annotation_before_save_event(
        ...
        AnnotationTransformEvent.assert_called_once_with(
            pyramid_request, annotation, presented
        )

    def test_it_notifies_before_save_event(
        self, AnnotationTransformEvent, es, notify, presenters, pyramid_request
    ):
    ...
    def presenters(self, patch):
        ...
        presenter.asdict.return_value = {
            "id": "test_annotation_id",
            "target": [{"source": "http://example.com/example"}],
        }
        return presenters
...
class TestBatchIndexer(object):
    def test_index_indexes_all_annotations_to_es(
        self, db_session, indexer, matchers, streaming_bulk, factories
    ):
        ...
        streaming_bulk.assert_called_once_with(
            indexer.es_client.conn,
            matchers.IterableWith(matchers.UnorderedList([ann_1, ann_2])),
            chunk_size=mock.ANY,
            raise_on_error=False,
            expand_action_callback=mock.ANY,
        )

    def test_index_skips_deleted_annotations_when_indexing_all(
        self, db_session, indexer, matchers, streaming_bulk, factories
    ):
        ann_1, ann_2 = factories.Annotation(), factories.Annotation()
    ...
    def test_index_indexes_filtered_annotations_to_es(
        self, db_session, indexer, matchers, streaming_bulk, factories
    ):
        ...
        streaming_bulk.assert_called_once_with(
            indexer.es_client.conn,
            matchers.IterableWith([ann_2]),
            chunk_size=mock.ANY,
            raise_on_error=False,
            expand_action_callback=mock.ANY,
        )

    def test_index_skips_deleted_annotations_when_indexing_filtered(
        self, db_session, indexer, matchers, streaming_bulk, factories
    ):
    ...
    def test_index_correctly_presents_bulk_actions(
        self, db_session, indexer, pyramid_request, streaming_bulk, factories
    ):
    ...
    def test_index_allows_to_set_op_type(
        self, db_session, es, pyramid_request, streaming_bulk, factories
    ):
        indexer = index.BatchIndexer(db_session, es, pyramid_request, op_type="create")
        annotation = factories.Annotation()
        db_session.add(annotation)
        db_session.flush()
        results = []
    ...
    def test_index_returns_failed_bulk_actions_for_default_op_type(
        self, db_session, indexer, streaming_bulk, factories
    ):
    ...
    def test_index_ignores_document_exists_errors_for_op_type_create(
        self, db_session, es, pyramid_request, streaming_bulk, factories
    ):
        indexer = index.BatchIndexer(db_session, es, pyramid_request, op_type="create")
        ...
        def fake_streaming_bulk(*args, **kwargs):
            for ann in args[1]:
                if ann.id in [ann_fail_1.id, ann_fail_2.id]:
                    error = "DocumentAlreadyExistsException[[index-name][1] [annotation][gibberish]: " "document already exists]"
...
def es():
    mock_es = mock.create_autospec(
        client.Client, instance=True, spec_set=True, index="hypothesis"
    )
