...
class TestSearch(object):
    ...
    def test_run_searches_replies(
        self, pyramid_request, _search_replies, _search_annotations
    ):
    ...
    def test_run_returns_search_results(
        self, pyramid_request, _search_annotations, _search_replies
    ):
        total = 4
        ...
        assert result == core.SearchResult(
            total, annotation_ids, reply_ids, aggregations
        )
    ...
    def test_search_annotations_parses_aggregation_results(self, pyramid_request):
        ...
        search.es.conn.search.return_value = {
            "hits": {"total": 0, "hits": []},
            "aggregations": {"foobar": {"foo": "bar"}, "bazqux": {"baz": "qux"}},
        }
    ...
    def test_search_annotations_excludes_replies_when_asked(
        ...
        assert (
            mock.call(query.TopLevelAnnotationsFilter())
            in search.builder.append_filter.call_args_list
        )
    ...
    def test_search_replies_adds_a_replies_matcher(self, pyramid_request, query):
        ...
        assert (
            mock.call(query.RepliesMatcher(["id-1", "id-2"]))
            in search.reply_builder.append_matcher.call_args_list
        )
    ...
    def test_search_replies_searches_replies_when_asked(self, pyramid_request):
        ...
        search.es.conn.search.return_value = {
            "hits": {"total": 2, "hits": [{"_id": "reply-1"}, {"_id": "reply-2"}]}
        }
    ...
    def test_search_replies_logs_warning_if_there_are_too_many_replies(
        ...
        search.es.conn.search.return_value = {
            "hits": {"total": 1100, "hits": [{"_id": "reply-1"}]}
        }
    ...
    def test_search_replies_works_with_stats_client(self, pyramid_request):
        search = core.Search(
            pyramid_request, stats=FakeStatsdClient(), separate_replies=True
        )
        # This should not raise
        search._search_replies(["id-1"])
...
@pytest.mark.parametrize(
    "filter_type",
    ["DeletedFilter", "AuthFilter", "UriFilter", "UserFilter", "GroupFilter"],
)
def test_default_querybuilder_includes_default_filters(
    filter_type, matchers, pyramid_request
):
...
@pytest.mark.parametrize("matcher_type", ["AnyMatcher", "TagsMatcher"])
...
def dummy_search_results(start=1, count=0, name="annotation"):
    ...
    for i in range(start, start + count):
        ...
        out["hits"]["hits"].append(
            {"_id": "id_{}".format(i), "_source": {"name": "{}_{}".format(name, i)}}
        )
