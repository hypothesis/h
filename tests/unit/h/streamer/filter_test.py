...
class TestFilterHandler:
    ...
    def test_set_filter_with_versioned_uri(self):
        """URIs with version suffixes produce versioned scope keys."""
        socket = FakeSocket()
        ...
        assert socket.filter_rows == (
            ("/uri", "httpx://example.com__v1"),
            ("/uri", "httpx://example.com__v2"),
        )

    def test_set_filter_with_v0_uri(self):
        """v0 in URI produces plain (unversioned) scope key."""
        socket = FakeSocket()
        ...
        assert socket.filter_rows == (
            ...
            ("/uri", "httpx://example.com__v3"),
        )

    def test_matching_versioned_annotation_only_matches_same_version(
        self, factories, db_session, storage
    ):
        """A versioned annotation only matches sockets filtering for that version."""
        ann = factories.Annotation(target_uri="http://example.com", version=3)
        storage.expand_uri.return_value = ["httpx://example.com"]

        # Socket filtering for version 3
        socket_v3 = FakeSocket()
        ...
        # Socket filtering for version 1
        socket_v1 = FakeSocket()
        SocketFilter.set_filter(
            socket_v1,
            {
                "match_policy": "include_any",
        ...
        # Socket filtering without version
        socket_plain = FakeSocket()
        SocketFilter.set_filter(
            socket_plain,
            {
                "match_policy": "include_any",
        ...
        assert socket_plain not in result

    def test_matching_unversioned_annotation_only_matches_plain_filter(
        self, factories, db_session, storage
    ):
        """An unversioned annotation only matches sockets with plain URI filter."""
        ann = factories.Annotation(target_uri="http://example.com", version=None)
        storage.expand_uri.return_value = ["httpx://example.com"]
        ...
        result = list(SocketFilter.matching([socket_plain, socket_v1], ann, db_session))

        assert socket_plain in result
        assert socket_v1 not in result

    def test_matching_v0_filter_matches_unversioned_annotation(
        self, factories, db_session, storage
    ):
        """v0 in filter matches unversioned annotations (v0 maps to plain URI)."""
        ann = factories.Annotation(target_uri="http://example.com", version=None)
        storage.expand_uri.return_value = ["httpx://example.com"]
        ...
        result = list(SocketFilter.matching([socket], ann, db_session))
