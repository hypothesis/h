...
ANNOTATION_MAPPING = {
    ...
    "properties": {
        ...
        "group": {"type": "string"},
        "thread_ids": {"type": "string", "index": "not_analyzed"},
    },
}
...
def update_index_settings(client):
    ...
    _update_index_mappings(
        client.conn, index, {client.t.annotation: ANNOTATION_MAPPING}
    )
...
def _update_index_mappings(conn, name, mappings):
    ...
    try:
        for doc_type, body in mappings.items():
            conn.indices.put_mapping(index=name, doc_type=doc_type, body=body)
