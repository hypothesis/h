...
def index(context, request):
    ...
    filter_terms = []
    if q:
        filter_terms.append(func.lower(Organization.name)
                                .like('%{}%'.format(q.lower())))

    return (request.db.query(Organization)
                      .filter(*filter_terms)
...
class OrganizationEditController(object):
    ...
    def delete(self):
        ...
        group_count = (self.request.db.query(models.Group)
                                      .filter_by(organization=org)
                                      .count())
        if group_count > 0:
            self.request.response.status_int = 400
            self.request.session.flash(
                _('Cannot delete organization because it is associated with {} groups'
                  .format(group_count)), 'error')
            return self._template_context()
