...
blocklist_index_fixtures = pytest.mark.usefixtures('models')
...
blocklist_add_fixtures = pytest.mark.usefixtures('models', 'blocklist_index')


@blocklist_add_fixtures
def test_blocklist_add_adds_uri_to_model(models):
    request = Mock(params={'add': 'test_uri'})

    admin.blocklist_add(request)
...
@blocklist_add_fixtures
def test_blocklist_add_returns_index(blocklist_index):
    request = Mock(params={'add': 'test_uri'})

    assert admin.blocklist_add(request) == blocklist_index.return_value


@blocklist_add_fixtures
def test_blocklist_add_flashes_error_if_uri_already_blocked(models):
    request = Mock(params={'add': 'test_uri'})
...
@blocklist_add_fixtures
def test_blocklist_add_returns_index_if_uri_already_blocked(
        models, blocklist_index):
    request = Mock(params={'add': 'test_uri'})
    models.Blocklist.side_effect = ValueError("test_error_message")
...
blocklist_remove_fixtures = pytest.mark.usefixtures('models', 'blocklist_index')


@blocklist_remove_fixtures
def test_blocklist_remove_deletes_model(models):
    request = Mock(params={'remove': 'test_uri'})

    admin.blocklist_remove(request)
...
@blocklist_remove_fixtures
def test_blocklist_remove_returns_index(blocklist_index):
    assert admin.blocklist_remove(Mock(params={'remove': 'test_uri'})) == (
        blocklist_index.return_value)
...
@pytest.fixture
def blocklist_index(request):
    patcher = patch('h.admin.blocklist_index', autospec=True)
    request.addfinalizer(patcher.stop)
    return patcher.start()
