...
def _matchdict(hashid=None, slug=None):
    """Return a matchdict like the one the group_read route receives."""
    return {"hashid": hashid or mock.sentinel.hashid,
            slug or "slug": mock.sentinel.slug}
...
def test_read_gets_group_by_hashid(Group):
    request = _mock_request(matchdict=_matchdict(hashid="foo"))
...
@read_fixtures
def test_read_if_not_a_member_gets_hashid_from_group(Group, User):
    """It should get the hashid from group.hashid().
...
join_fixtures = pytest.mark.usefixtures('User', 'Group', 'logic')
...
@join_fixtures
def test_join_uses_hashid_from_matchdict_to_get_group(Group):
