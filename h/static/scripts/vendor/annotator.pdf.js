...
(function() {
  ...
  window.PDFTextMapper = (function(_super) {
    ...
    PDFTextMapper.prototype._PDFjsVersion = {
      compareVersions: function(v1, v2) {
        var i, _i, _ref;
        if (!(Array.isArray(v1) && Array.isArray(v2))) {
          throw new Error("Expecting arrays!");
        }
      ...
      toNumeric: function(version) {
        return (version != null ? version : this.get()).split(".").map(parseFloat);
      },
      compareTo: function(testVersion) {
        var currentVersion;
        currentVersion = this.toNumeric();
        if (!Array.isArray(testVersion)) {
          testVersion = this.toNumeric(testVersion);
        }
        return this.compareVersions(currentVersion, testVersion);
      },
      isOlderThan: function(version) {
        return this.compareTo(version) === -1;
      },
      isExactly: function(version) {
    ...
    function PDFTextMapper() {
      ...
      this.setEvents();
      if (this._PDFjsVersion.isAtLeast("1.0.822")) {
        this._viewer.container.classList.add("has-transparent-text-layer");
      }
    }
