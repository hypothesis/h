...
describe('SignupForm', () => {
  ...
  const getElements = wrapper => {
    return {
      ...
      privacyHiddenInput: wrapper.find('input[name="privacy_accepted"]'),
      commsHiddenInput: wrapper.find('input[name="comms_opt_in"]'),
  ...
  it('renders signup form', () => {
    ...
    assert.equal(privacyCheckbox.prop('checked'), false);
    assert.equal(commsCheckbox.prop('checked'), false);
    assert.equal(privacyHiddenInput.prop('value'), 'false');
    assert.equal(commsHiddenInput.prop('value'), 'false');
  });
  ...
  it('updates privacy checkbox when clicked', () => {
    ...
    act(() => {
      elements.privacyCheckbox.prop('onChange')(mockEvent);
    });
    wrapper.update();
    const updatedElements = getElements(wrapper);

    assert.equal(updatedElements.privacyCheckbox.prop('checked'), true);
    assert.equal(updatedElements.privacyHiddenInput.prop('value'), 'true');
  });
  ...
  it('updates communication opt-in checkbox when clicked', () => {
    ...
    act(() => {
      elements.commsCheckbox.prop('onChange')(mockEvent);
    });
    wrapper.update();
    const updatedElements = getElements(wrapper);

    assert.equal(updatedElements.commsCheckbox.prop('checked'), true);
    assert.equal(updatedElements.commsHiddenInput.prop('value'), 'true');
  });
