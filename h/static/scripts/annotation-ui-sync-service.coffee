...
class AnnotationUISyncService
  ...
  this.$inject = ['$document', '$window', 'store', 'annotationUI', '$rootScope']
  constructor:   ( $document,   $window,   store ,  annotationUI,   $rootScope) ->
    ...
    # Proxy drag events through to the host frame.
    $document.context.addEventListener 'dragover', (event) =>
      @host?.notify
        method: 'dragFrame'
